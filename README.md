...
Я изучил git, на практике закрепил: push, merge, pull и fetch. Также открыл для себя удобное расширение git graph, что позволяет более красиво и понятние следить за историей изменений git. Я научился(Ну больше за предыдущие пары) решать конфликты в git-е. Это натолкнуло использовать это на практикте, в моём случае, синхронизировать конфиги в моей сборке Arch Linux. Трудностей почти не было, разве что, я иногда забывал добавить файлы в коммит и VS code постоянно открывал отдельное окно в редакторе чтобы я убедился в том что я коммучу.
